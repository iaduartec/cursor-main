...
export default async function AdminPostsPage() {
  ...
  return (
    <div className='max-w-6xl mx-auto p-6'>
      ...
      <form
        action={upsert}
        className='grid grid-cols-1 md:grid-cols-2 gap-3 p-4 border rounded mb-8'
      >
        <input
          name='slug'
          placeholder='slug'
        ...
        <textarea
          ...
          className='border rounded px-2 py-2 md:col-span-2 h-40'
        />
        ...
        <tbody>
          {items.map(p => (
            <tr key={p.slug} className='border-b'>
              ...
              <td>{p.title}</td>
              <td>{p.date ? new Date(p.date).toLocaleString('es-ES') : ''}</td>
              <td>{p.category}</td>
              <td>
                <form action={remove}>
