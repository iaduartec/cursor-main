...
async function verifyNeonConnection() {
  ...
  if (!connectionString) {
    console.error('❌ No se encontró POSTGRES_URL, DATABASE_URL o NEON_DATABASE_URL en las variables de entorno');
    process.exit(1);
  }
  ...
  try {
    ...
    console.log(`📝 Posts en base de datos: ${Number(postsRows?.[0]?.count ?? 0)}`);

    const servicesRows = await sql`SELECT COUNT(*) as count FROM services`;
    console.log(`🔧 Servicios en base de datos: ${Number(servicesRows?.[0]?.count ?? 0)}`);

    const projectsRows = await sql`SELECT COUNT(*) as count FROM projects`;
    console.log(`📁 Proyectos en base de datos: ${Number(projectsRows?.[0]?.count ?? 0)}`);

    const streamsRows = await sql`SELECT COUNT(*) as count FROM streams`;
    console.log(`📹 Streams en base de datos: ${Number(streamsRows?.[0]?.count ?? 0)}`);
